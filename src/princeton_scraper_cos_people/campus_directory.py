...
CLASS_RESULTS_DETAILS_NETID = "expanded-details-value"
CLASS_RESULTS_DETAILS_EMAIL = "people-search-email"
...
def find_netid_from_princeton_email(princeton_email: str, fast: bool = False) -> typing.Optional[str]:
    ...
    tag_netid = tag_netid_label.find_next_sibling("span", attrs={"class": CLASS_RESULTS_DETAILS_NETID})
...
def find_princeton_email_from_netid(netid: str) -> typing.Optional[str]:
    """
    Returns the campus email (@princeton.edu), given a valid Princeton NetID; this
    places a query with the central campus directory as available publicly from:
    `https://search.princeton.edu`.

    :param netid: A valid Princeton NetID.
    :return: The campus email of the person whose NetID was provided as an argument.
    """

    url = PRINCETON_CAMPUS_DIRECTORY_NETID_SEARCH_URL.format(netid)
    ...
    first = raw_items[0]
    tag_email = first.find("div", attrs={"class": CLASS_RESULTS_DETAILS_EMAIL})
    if tag_email is None:
        return

    return tag_email.text.strip().lower()
